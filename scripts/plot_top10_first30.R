...
outfile <- if (length(args) >= 2) args[2] else "results/gene_top10_first30.png"
...
# Subset to first 30 conditions (or fewer if file has less)
nconds <- min(30, ncol(expr))
expr <- expr[, seq_len(nconds), drop = FALSE]
...
summary_tbl <- tibble::tibble(gene = genes, mean = means, sd = sds, cv = cv)
...
long <- expr_top %>% tidyr::pivot_longer(-gene, names_to = "condition", values_to = "expression")
